...
function selectHousing(type, requiredDown, monthlyCost) {
    // Use the stored affordability level instead of recalculating
    const affordableLevel = gameState.affordabilityLevel;
    ...
    // Handle buy option
        ...
        const houseOptions = [
            ...
            { level: 3, cost: 476.04, price: 79000 },
            { level: 2, cost: 266.20, price: 64000 }
        ];
        ...
        if (selectedHouse) {
            ...
            console.log("No matching house found for affordability level:", affordableLevel);
            alert("You cannot afford this house with your current down payment.");
            return;
        }
    }
    // Update UI
...
function confirmChoice() {
    ...
    if (selectedOption === 'buy') {
        ...
        // Debug logging
        console.log('Current Balance:', gameState.currentBalance);
        console.log('Required Down Payment:', downPayment);
        ...
        gameState.carPurchasePrice = purchasePrice;

        // Show final results
        document.getElementById("chosen-vehicle").textContent = selectedHovercraft.name;
        document.getElementById("chosen-type").textContent = 'Purchase';
        document.getElementById("chosen-down").textContent = formatCurrency(downPayment);
...
function playAgain() {
    localStorage.clear(); // Clear the game state
